...
-- BCrypt hash for 'password': $2a$10$69HskhMzpFvU9ws9XBmMBuF3jmALJ1m7O8DPmwujun7LxahZqUKYi

-- Seed Users
INSERT INTO users (username, password, full_name, email, role, points, location, bio, language, enabled, deleted)
VALUES 
    ('admin', '$2a$10$69HskhMzpFvU9ws9XBmMBuF3jmALJ1m7O8DPmwujun7LxahZqUKYi', 'Academy Admin', 'admin@ruraledu.com', 'ADMIN', 1000, 'Pune', 'Chief Administrator of RuralEduHub Platform', 'English', true, false),
    ('aryan', '$2a$10$69HskhMzpFvU9ws9XBmMBuF3jmALJ1m7O8DPmwujun7LxahZqUKYi', 'Aryan Sharma', 'aryan@student.com', 'STUDENT', 500, 'Pune', 'Aspiring Agronomist and Technology Enthusiast', 'Hindi', true, false),
    ('teacher', '$2a$10$69HskhMzpFvU9ws9XBmMBuF3jmALJ1m7O8DPmwujun7LxahZqUKYi', 'Dr. Savita Rao', 'savita@teacher.com', 'TEACHER', 1500, 'Nagpur', 'Senior Agricultural Scientist with 15 years of experience', 'Marathi', true, false),
    ('parent1', '$2a$10$69HskhMzpFvU9ws9XBmMBuF3jmALJ1m7O8DPmwujun7LxahZqUKYi', 'Priya Sharma', 'priya@parent.com', 'PARENT', 0, 'Pune', 'Parent of Aryan Sharma', 'Hindi', true, false)
...
SELECT 
    ...
    false
FROM dual 
WHERE NOT EXISTS (SELECT 1 FROM courses WHERE title = 'Sustainable Organic Farming' AND deleted = false);
...
SELECT 
    ...
    false
FROM dual 
WHERE NOT EXISTS (SELECT 1 FROM courses WHERE title = 'Digital Literacy for Rural Entrepreneurs' AND deleted = false);
...
FROM courses c 
WHERE c.title = 'Sustainable Organic Farming' AND c.deleted = false
...
SELECT 
    ...
    'Pest control', 'Nutrient absorption', 'Water storage', 'Seed dispersal', 'B', 
    q.id
FROM quizzes q 
...
INSERT INTO questions (content, option_a, option_b, option_c, option_d, correct_answer, quiz_id)
SELECT 
    'Which soil type is best for organic farming?', 
    'Sandy soil', 'Loamy soil', 'Clay soil', 'Rocky soil', 'B', 
    q.id
FROM quizzes q 
JOIN courses c ON q.course_id = c.id 
WHERE c.title = 'Sustainable Organic Farming' AND c.deleted = false
...
SELECT 
    'What is composting?', 
    'Burning waste', 'Decomposing organic matter', 'Mixing chemicals', 'Plastic recycling', 'B', 
    q.id
FROM quizzes q 
JOIN courses c ON q.course_id = c.id 
WHERE c.title = 'Sustainable Organic Farming' AND c.deleted = false
